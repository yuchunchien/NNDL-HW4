...
from hw4_utils import load_data, contextwin, shuffle, conlleval, check_dir, shuffle_rnn
...
class RNNSLU2(object):
    ...
    def __init__(self, nh, nh2, nc, ne, de, cs, normal=True):
        ...
        self.wx = theano.shared(name='wx',
                                ...
                                (de * cs, nh))
                                .astype(theano.config.floatX))
        self.wx_2 = theano.shared(name='wx_2',
                                  value=0.2 * numpy.random.uniform(-1.0, 1.0,
                                  (nh, nh2))
                                  .astype(theano.config.floatX))
        self.wh = theano.shared(name='wh',
                                ...
                                (nh, nh))
                                .astype(theano.config.floatX))
        self.wh_2 = theano.shared(name='wh_2',
                                  value=0.2 * numpy.random.uniform(-1.0, 1.0,
                                  (nh2, nh2))
                                  .astype(theano.config.floatX))
        ...
        self.bh_2 = theano.shared(name='bh_2',
        ...
        self.h0 = theano.shared(name='h0',
                                value=numpy.zeros(nh,
                                dtype=theano.config.floatX))
        self.h0_2 = theano.shared(name='h0_2',
                                  value=numpy.zeros(nh2,
                                  dtype=theano.config.floatX))

        # bundle
        self.params = [self.emb, self.wx, self.wx_2, self.wh, self.wh_2, self.w,
                       self.bh, self.bh_2, self.b, self.h0, self.h0_2]
        ...
        def recurrence(x_t, h_tm1, h_tm1_2):
            h_t = T.nnet.sigmoid(T.dot(x_t, self.wx) + T.dot(h_tm1, self.wh) + self.bh)
            
            h_t_2 = T.nnet.sigmoid(T.dot(h_t, self.wx_2) + T.dot(h_tm1_2, self.wh_2) + self.bh_2)
            
            s_t = T.nnet.softmax(T.dot(h_t_2, self.w) + self.b)
            
            return [h_t, h_t_2, s_t]

        [h, h_2, s], _ = theano.scan(fn=recurrence,
                                     sequences=x,
                                     outputs_info=[self.h0, self.h0_2, None],
                                     n_steps=x.shape[0])
...
class RNNSLU_n(object):
    ...
    def __init__(self, nh, nc, ne, de, cs, normal=True):
        ...
        def recurrence(x_t, h_tm1):
            A = T.dot(x_t, self.wx) + T.dot(h_tm1, self.wh) + self.bh
            A_mean = A.mean()
            A_std  = A.std()
            A_normal = (self.G/A_std) * (A-A_mean) + self.B
            h_t = T.nnet.sigmoid(A_normal)
